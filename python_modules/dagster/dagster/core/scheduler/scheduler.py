...
class DagsterDaemonScheduler(Scheduler, ConfigurableClass):
    ...
    Args:
        ...
        tick_retries (int): For each schedule tick that raises an error, how many times to retry
            that tick (defaults to 0).
    """

    def __init__(self, max_catchup_runs=DEFAULT_MAX_CATCHUP_RUNS, tick_retries=0, inst_data=None):
        self.max_catchup_runs = check.opt_int_param(
            max_catchup_runs, "max_catchup_runs", DEFAULT_MAX_CATCHUP_RUNS
        )
        self.tick_retries = check.opt_int_param(tick_retries, "tick_retries", 0)
    ...
    def config_type(cls):
        return {
            ...
            "tick_retries": Field(IntSource, is_required=False),
        }
