...
def execute_scheduler_iteration(instance, workspace, logger, max_catchup_runs, tick_retries):
    end_datetime_utc = pendulum.now("UTC")
    yield from launch_scheduled_runs(
        instance, workspace, logger, end_datetime_utc, max_catchup_runs, tick_retries
    )
...
def launch_scheduled_runs_for_schedule(
    ...
    if latest_tick:
        if latest_tick.status == JobTickStatus.STARTED:
            ...
            and latest_tick.failure_count <= tick_retries
        ):
